...
export async function handleSendEmail(data: z.infer<typeof formSchema>) {
  const { name, email, company, project } = data;
  ...
  const mailOptions = {
    ...
    to: 'development@techgetafrica.com', // This is where you'll receive the inquiries
    replyTo: email,
    subject: `New Development Inquiry from ${name}`,
    html: `
